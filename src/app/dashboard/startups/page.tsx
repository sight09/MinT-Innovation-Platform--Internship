  ...
  return (
    <div style={{ background: 'var(--surface-base)', minHeight: '100vh', paddingBottom: 64 }}>
      ...
      <div className="container" style={{ paddingTop: 40 }}>
        ...
        {/* Filter / Search Bar */}
        <form method="GET" action="/dashboard/startups" style={{
          background: 'var(--surface-card)', border: '1px solid var(--surface-border)',
          borderRadius: 16, padding: '14px 16px', marginBottom: 32,
          ...
          <div style={{ position: 'relative', flex: '1 1 240px', minWidth: 0 }}>
            ...
            <input
              ...
              placeholder="Search startups, problems, technologies..."
            />
          </div>

          <select name="sector" defaultValue={sector || ''} className="input select" style={{ flex: '0 0 160px' }}>
            <option value="">All Sectors</option>
            {sectors.map(s => <option key={s} value={s}>{s}</option>)}
          </select>

          <select name="stage" defaultValue={stage || ''} className="input select" style={{ flex: '0 0 150px' }}>
            <option value="">All Stages</option>
            {stages.map(st => <option key={st} value={st}>{getStageLabel(st as any)}</option>)}
          </select>

          <button type="submit" className="btn btn-primary" style={{ flex: '0 0 auto', whiteSpace: 'nowrap', display: 'flex', alignItems: 'center', gap: 6 }}>
